...
- `GET /user/starred` — authenticated viewer's starred repositories
- `GET /user/subscriptions` — authenticated viewer's watched repositories
...
- **`personal_repo_releases`** — resolves the authenticated viewer's starred
  and/or watched repositories, filters the collection, and tracks releases
...
- `collections` — any combination of `starred` and `watched`; personal sources
  require authentication
- `keywords` — case-insensitive whole words or phrases matched against repository
  name, description, and topics; an empty list includes the whole collection
- `recently_pushed_days` — excludes dormant repositories before release fetching
- `max_repositories` — caps repositories after filtering, newest activity first
- `include_prereleases` / `include_drafts` — release-channel controls
- `include_repo_metadata` — adds stars, forks, topics, description, and license
  to scoring metadata
- `include_archived` / `include_forks` — personal collection controls

**Authentication**: Set `GITHUB_TOKEN` in your environment for personal
collections and higher rate limits (5000 req/hr vs 60 without). Static public
repository releases still work without authentication.
