...
With `selection_mode: "all"`, profile thresholds and semantic topic removal are
bypassed so every analyzed candidate remains visible. `detail_mode: "compact"`
skips enrichment and can render those candidates as a score-ranked source feed.

## Enrichment And Output

Selected items are enriched according to the profile's `enrichment.md` prompt
and block contract when `detail_mode` is `full`. A block can call a tool only
when that tool is declared in the block's `tools` list. The only built-in tool
is `web_search`. Compact mode uses the first-pass analysis summary instead.
