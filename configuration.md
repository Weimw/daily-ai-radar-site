...
| Codex subscription | _(empty; uses `codex login`)_ |
...
This local-only backend invokes `codex exec` and uses the ChatGPT account that
was authenticated with `codex login`. It is not an OpenAI API integration;
ChatGPT/Codex subscription usage and API billing are separate. `default` uses
the model selected by the installed Codex client. Use a concrete Codex model ID
to pin it. Temperature and token-limit fields are ignored by this backend.
...
```json
{
  "sources": {
    "github": [
      ...
      {
        "type": "personal_repo_releases",
        "collections": ["starred", "watched"],
        "keywords": ["ai", "llm", "agent", "machine learning", "inference"],
        "recently_pushed_days": 365,
...
`personal_repo_releases` reads the authenticated GitHub viewer's collections at
run time. `keywords` uses whole-word/phrase matching against repository names,
descriptions, and topics. Repositories are deduplicated across starred, watched,
and explicit `repo_releases` entries. `GITHUB_TOKEN` is required for personal
collections. See [Source Scrapers](scrapers.md#github) for all controls.
...
```json
{
  "digest": {
    ...
    "project_diversity": {
      "enabled": true,
      "max_items_per_project": 1,
      "aggregate_github_releases": true
    },
...
- `selection_mode`: `filtered` applies profile thresholds and semantic topic
  deduplication. `all` retains every analyzed candidate before deterministic
  project diversity is applied.
- `group_by`: Render sections by processing `profile` or source family.
- `detail_mode`: `full` enriches and renders detailed items. `compact` skips
  enrichment and renders ranked titles with their analysis summaries.
- `max_items`: Optional final cap after all group limits are applied
- `project_diversity`: Optional deterministic project-level diversity rule.
  `max_items_per_project` limits recognized projects after score sorting, while
  `aggregate_github_releases` bundles same-repository release tags into one
  candidate before AI analysis. Raw fetched items are not mutated.
...
and enabled project diversity disables balanced digest limits; configured
profile thresholds still apply.
